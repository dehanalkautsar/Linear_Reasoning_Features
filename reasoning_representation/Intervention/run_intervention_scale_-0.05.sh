...
LOG_FILE="${LOGS_DIR}/intervention_scale_-0.05${LANG_SUFFIX}.log"
...
python "${SCRIPT_DIR}/features_intervention.py" \
  ...
  --lang "${LANG_CODE}" \
  --label_subset "memorization" \
  --Intervention True \
  --scale -0.05 \
  --batch_size "${BATCH_SIZE:-46}" --metrics_out "${METRICS_DIR}/intervention_scale_-0.05_metrics${LANG_SUFFIX}.json" \
  2>&1 | tee "${LOG_FILE}"
