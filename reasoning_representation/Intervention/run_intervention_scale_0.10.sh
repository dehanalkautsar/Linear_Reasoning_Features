...
LOG_FILE="${LOGS_DIR}/intervention_scale_0.10${LANG_SUFFIX}.log"
...
python "${SCRIPT_DIR}/features_intervention.py" \
  ...
  --Intervention True \
  --scale 0.10 \
  --batch_size "${BATCH_SIZE:-46}" --metrics_out "${METRICS_DIR}/intervention_scale_0.10_metrics${LANG_SUFFIX}.json" \
  2>&1 | tee "${LOG_FILE}"
