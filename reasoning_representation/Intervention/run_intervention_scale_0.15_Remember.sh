...
LOG_FILE="${LOGS_DIR}/intervention_scale_0.15_Remember${LANG_SUFFIX}.log"
...
python "${SCRIPT_DIR}/features_intervention.py" \
  ...
  --lang "${LANG_CODE}" \
  --label_subset "remember" \
  --Intervention True \
  --scale 0.15 \
  --batch_size "${BATCH_SIZE:-46}" \
  --metrics_out "${METRICS_DIR}/intervention_scale_0.15_Remember_metrics${LANG_SUFFIX}.json" \
  2>&1 | tee "${LOG_FILE}"
